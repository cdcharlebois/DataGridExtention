    ...
    return declare(null, {
        ...
        setSortOrder: function() {
            ...
            if (this.grid.constraintsFilled()) {
                this.grid._dataSource.reload(dojo.hitch(this.grid, this.grid.refreshGrid));
            }
        },
        ...
        endResize: function(evt) {
            ...
            for (var i = 0; i < cols.length; i++) {
                ...
                var width = Math.round(cols[i].width / total * 100)
                    .toString() + "%";
                for (var j = 0; j < this.gridAttributes.length; j++) {
                ...
                for (var j = 0; j < this.gridAttributesStore.length; j++) {
                    if (this.gridAttributesStore[j].tag === this.gridAttributes[i].tag) {
                        this.gridAttributesStore[j].display.width = Math.round(cols[i].width / total * 100)
                            .toString() + "%";
                    }
                }
            }
        ...
        loadContextMenu: function() {
            ...
            dojo.query("body")
                .connect("oncontextmenu", dojo.hitch(this, function(evt) {
                ...
                var horMover = declare([Mover], {
                    onMouseMove: function(evt) {
                        ...
                        if (evt.pageX - this.host.startPosX > this.host.minX) {
                            var w = domGeom.position(this.host.node)
                                .w;
        ...
        headerMoveStart: function(evt) {
            ...
            evt.host.startPosX = domGeom.position(evt.node)
                .x - evt.node.offsetLeft;
            var headers = this.grid.gridHeadNode.children[0].children;
            for (var i = 0; i < headers.length; i++) {
                this.gridAttributes[i].i = i; // set original sort order
            }
            evt.host.minX = domGeom.position(evt.node.parentNode)
                .x - domGeom.position(evt.node.parentNode)
                .x;
            evt.host.maxX = domGeom.position(evt.node.parentNode)
                .x + domGeom.position(evt.node.parentNode)
                .w - domGeom.position(evt.node.parentNode)
                .x;
        ...
        headerClick: function(tag, node, evt) {
            ...
            if (evt.button === dojo.mouseButtons.RIGHT) {
                ...
                var x = evt.pageX - 5,
                    menuWidth = domGeom.position(this.contextMenu)
                    .w,
